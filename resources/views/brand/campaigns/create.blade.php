...
@section('title', $isEdit ? 'Ubah Kampanye' : 'Buat Kampanye Baru')
...
<div class="w-full space-y-6 pb-20 pt-2">
    ...
    <div class="bg-[#111111] border border-[#1f1f1f] rounded-[1.5rem] relative overflow-hidden">
        ...
        <form action="{{ $isEdit ? route('brand.campaigns.update', $campaign) : route('brand.campaigns.store') }}" method="POST" enctype="multipart/form-data" class="relative z-10 flex flex-col">
            @csrf
            @if($isEdit)
                @method('PATCH')
            @endif
            ...
            <div class="p-6 lg:p-8 border-b border-white/5 space-y-8">
                ...
                <div class="flex flex-col gap-6">
                    <div class="w-full">
                        ...
                        <input type="text" name="title" value="{{ old('title', $campaign->title ?? '') }}" class="w-full bg-[#000] border border-[#27272a] rounded-2xl px-5 py-3.5 text-sm text-white font-medium transition-all duration-200 focus:outline-none focus:border-violet-400 focus:ring-4 focus:ring-violet-500/15 placeholder:text-[#52525b] placeholder:font-medium" placeholder="Contoh: Promo Flash Sale Spesial Lebaran" required>
                    </div>
                    ...
                    <div class="flex flex-col sm:flex-row gap-6 w-full">
                        <div class="flex-1">
                            ...
                            <select name="type" class="w-full bg-[#000] border border-[#27272a] rounded-2xl px-5 py-3.5 text-sm text-white font-medium transition-all duration-200 focus:outline-none focus:border-violet-400 focus:ring-4 focus:ring-violet-500/15 cursor-pointer appearance-none">
                                <option value="video" @selected(old('type', $campaign->type ?? 'video') === 'video')>UGC Video Biasa (Kreator mereview sendiri)</option>
                                <option value="clip" @selected(old('type', $campaign->type ?? '') === 'clip')>Clip Video (Reupload materi mentah + voiceover)</option>
                            </select>
                        </div>
                        <div class="flex-1">
                            ...
                            <input type="number" name="slots" value="{{ old('slots', $campaign->slots ?? '') }}" class="w-full bg-[#000] border border-[#27272a] rounded-2xl px-5 py-3.5 text-sm text-white font-medium transition-all duration-200 focus:outline-none focus:border-violet-400 focus:ring-4 focus:ring-violet-500/15 placeholder:text-[#52525b] placeholder:font-medium" placeholder="Contoh: 50" required>
                        </div>
                    </div>

                    <div class="w-full">
                        <label class="block text-xs font-extrabold text-[#a1a1aa] mb-2 tracking-wide">Banner / Gambar Thumbnail @unless($isEdit)<span class="text-violet-500">*</span>@endunless</label>
                        <div class="border border-dashed border-white/15 rounded-2xl p-10 text-center cursor-pointer bg-black/50 transition-all duration-200 hover:border-violet-500/50 hover:bg-violet-500/5 hover:shadow-[inset_0_0_0_1px_rgba(139,92,246,0.1)] flex flex-col items-center justify-center" onclick="document.getElementById('thumbnail-upload').click()">
                            ...
                            <input type="file" name="thumbnail" class="hidden" accept="image/*" id="thumbnail-upload" @unless($isEdit) required @endunless onchange="document.getElementById('file-name').innerText = this.files[0] ? this.files[0].name : 'Belum ada file dipilih'">
                            ...
                            <p class="text-xs text-violet-400 mt-3 font-semibold" id="file-name">{{ $isEdit && $campaign->thumbnail ? 'Thumbnail saat ini tersimpan' : '' }}</p>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="flex flex-col gap-6">
                    ...
                    <div class="flex flex-col sm:flex-row gap-6 w-full">
                        ...
                        <div class="flex-1">
                            ...
                            <input type="date" name="deadline" value="{{ old('deadline', isset($campaign) && $campaign->deadline ? \Illuminate\Support\Carbon::parse($campaign->deadline)->format('Y-m-d') : '') }}" class="w-full bg-[#000] border border-[#27272a] rounded-2xl px-5 py-3.5 text-sm text-white font-medium transition-all duration-200 focus:outline-none focus:border-violet-400 focus:ring-4 focus:ring-violet-500/15 placeholder:text-[#52525b] placeholder:font-medium" required>
                        </div>
                    </div>
                    ...
                    <div class="flex flex-col sm:flex-row gap-6 w-full">
                        <div class="flex-1">
                            ...
                            <input type="text" name="video_length" value="{{ old('video_length', $campaign->video_length ?? '') }}" class="w-full bg-[#000] border border-[#27272a] rounded-2xl px-5 py-3.5 text-sm text-white font-medium transition-all duration-200 focus:outline-none focus:border-violet-400 focus:ring-4 focus:ring-violet-500/15 placeholder:text-[#52525b] placeholder:font-medium" placeholder="Contoh: 30 - 60 Detik" required>
                        </div>
                        <div class="flex-1">
                            ...
                            <input type="url" name="link" value="{{ old('link', $campaign->link ?? '') }}" class="w-full bg-[#000] border border-[#27272a] rounded-2xl px-5 py-3.5 text-sm text-white font-medium transition-all duration-200 focus:outline-none focus:border-violet-400 focus:ring-4 focus:ring-violet-500/15 placeholder:text-[#52525b] placeholder:font-medium" placeholder="Link Affiliate / Link Toko yang dicantumkan kreator" required>
                        </div>
                    </div>
                    ...
                    <div class="w-full">
                        ...
                        <select name="platform" class="w-full bg-[#000] border border-[#27272a] rounded-2xl px-5 py-3.5 text-sm text-white font-medium transition-all duration-200 focus:outline-none focus:border-violet-400 focus:ring-4 focus:ring-violet-500/15 cursor-pointer appearance-none">
                            <option value="all" @selected(old('platform', $campaign->platform ?? 'all') === 'all')>Semua Platform (TikTok, IG, YT)</option>
                            <option value="tiktok" @selected(old('platform', $campaign->platform ?? '') === 'tiktok')>Khusus TikTok</option>
                            <option value="ig_reels" @selected(old('platform', $campaign->platform ?? '') === 'ig_reels')>Khusus Instagram Reels</option>
                            <option value="yt_shorts" @selected(old('platform', $campaign->platform ?? '') === 'yt_shorts')>Khusus YouTube Shorts</option>
                        </select>
                    </div>
                </div>
            </div>
            ...
            <div class="p-6 lg:p-8 space-y-8 bg-[#0a0a0a]">
                ...
                <div class="rounded-xl p-5 flex gap-4 items-start relative overflow-hidden bg-violet-500/5 border border-violet-500/20">
                    ...
                    <div class="relative z-10 w-full">
                        ...
                        <p class="text-[11px] text-slate-400 leading-relaxed mb-4 pr-8">Semua budget yang Anda setorkan akan ditahan dengan aman oleh sistem pintar Clipfluence. Saldo hanya akan dipotong ketika kreator sukses memproduksi views yang *tervalidasi* oleh algoritma API kami.</p>
                ...
                <div class="flex flex-col md:flex-row gap-6 w-full relative">
                    <div class="flex-1">
                        ...
                        <div class="relative">
                            ...
                            <input type="number" name="budget" value="{{ old('budget', $campaign->budget ?? '') }}" class="w-full bg-[#000] border border-[#27272a] rounded-2xl px-5 py-3.5 text-sm text-white font-medium transition-all duration-200 focus:outline-none focus:border-violet-400 focus:ring-4 focus:ring-violet-500/15 placeholder:text-[#52525b] placeholder:font-medium font-black text-lg tracking-wide pl-12" placeholder="0" min="0" required>
                        </div>
                    </div>
            ...
            <div class="p-6 bg-[#050505] border-t border-white/5 flex flex-col sm:flex-row items-center justify-end gap-3 sticky bottom-0 z-20 shadow-[0_-15px_40px_rgba(0,0,0,0.8)]">
                ...
                <button type="submit" name="action" value="draft" class="px-6 py-3.5 rounded-2xl font-bold text-sm bg-neutral-900 border border-neutral-700 text-slate-300 hover:bg-neutral-800 hover:text-white transition w-full sm:w-auto">
                    {{ $isEdit ? 'Simpan sebagai Draft' : 'Simpan Draft' }}
                </button>
                <button type="submit" name="action" value="active" class="bg-gradient-to-br from-violet-600 to-fuchsia-600 text-white px-10 py-3.5 rounded-2xl text-sm font-extrabold inline-flex items-center justify-center gap-2 transition-all duration-200 shadow-[0_8px_20px_rgba(124,58,237,0.25)] hover:shadow-[0_8px_30px_rgba(124,58,237,0.4)] hover:-translate-y-0.5 cursor-pointer border-none outline-none w-full sm:w-auto shadow-[0_0_20px_rgba(139,92,246,0.2)]">
                    <i data-lucide="rocket" class="w-5 h-5"></i> {{ $isEdit ? 'Simpan & Aktifkan' : 'Luncurkan Kampanye' }}
                </button>
            </div>
